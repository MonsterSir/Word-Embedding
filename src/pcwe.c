...
void InitUnigramTable()
{
    ...
    for (a = 0; a < vocab_size; a++)
        train_words_pow += pow(vocab[a].cn, power); // 计算频率幂次和
    i = 0;
...
void LearnVocabFromTrainFile()
{
    ...
    long long a, i;
    for (a = 0; a < vocab_hash_size; a++)
        vocab_hash[a] = -1; // initialize vocab_hash array
        ...
        if (cw)
        {
            ...
            if (negative > 0)
                for (d = 0; d < negative + 1; d++)
                {
                    ...
                    { // average context composition model
                        real f = 0, g = 0;
...
void TrainModel()
{
    ...
    for (a = 0; a < vocab_size; a++)
    {
        ...
        if (binary)
            for (b = 0; b < layer1_size; b++)
                fwrite(&synword[a * layer1_size + b], sizeof(real), 1, fo);
        else
            for (b = 0; b < layer1_size; b++)
                fprintf(fo, "%lf ", synword[a * layer1_size + b]);
        fprintf(fo, "\n");
    }
    fclose(fo);
    wchar_t ch[10];
    if (strlen(output_char))
    {
        ...
        for (a = 0; a < CHAR_SIZE; a++)
        {
            ...
            if (binary)
                for (b = 0; b < layer1_size; b++)
                    fwrite(&synchar[a * layer1_size + b], sizeof(real), 1, fo);
            else
                for (b = 0; b < layer1_size; b++)
                    fprintf(fo, "%lf ", synchar[a * layer1_size + b]);
            fprintf(fo, "\n");
        }
        fclose(fo);
    }
    if (strlen(output_comp))
    {
        ...
        for (a = 0; a < comp_size; a++)
        {
            ...
            if (binary)
                for (b = 0; b < layer1_size; b++)
                    fwrite(&syncomp[a * layer1_size + b], sizeof(real), 1, fo);
            else
                for (b = 0; b < layer1_size; b++)
                    fprintf(fo, "%lf ", syncomp[a * layer1_size + b]);
            fprintf(fo, "\n");
        }
        fclose(fo);
    }
    ...
    for (a = 0; a < pron_size; a++)
    {
        ...
        if (binary)
            for (b = 0; b < layer1_size; b++)
                fwrite(&synpron[a * layer1_size + b], sizeof(real), 1, fo);
        else
            for (b = 0; b < layer1_size; b++)
                fprintf(fo, "%lf ", synpron[a * layer1_size + b]);
        fprintf(fo, "\n");
    }
    fclose(fo);
...
int main(int argc, char **argv)
{
    ...
    if ((i = ArgPos((char *)"-alpha", argc, argv)) > 0)
        alpha = atof(argv[i + 1]);
    if ((i = ArgPos((char *)"-output-word", argc, argv)) > 0)
        strcpy(output_word, argv[i + 1]);
    if ((i = ArgPos((char *)"-output-char", argc, argv)) > 0)
        strcpy(output_char, argv[i + 1]);
    if ((i = ArgPos((char *)"-output-comp", argc, argv)) > 0)
        strcpy(output_comp, argv[i + 1]);
    if ((i = ArgPos((char *)"-output-pron", argc, argv)) > 0)
        strcpy(output_pron, argv[i + 1]);
    if ((i = ArgPos((char *)"-comp", argc, argv)) > 0)
        strcpy(comp_file, argv[i + 1]);
    if ((i = ArgPos((char *)"-char2comp", argc, argv)) > 0)
        strcpy(char2comp_file, argv[i + 1]);
    if ((i = ArgPos((char *)"-pron", argc, argv)) > 0)
        strcpy(pron_file, argv[i + 1]);
    if ((i = ArgPos((char *)"-word2pron", argc, argv)) > 0)
        strcpy(word2pron_file, argv[i + 1]);
    ...
    if ((i = ArgPos((char *)"-join-type", argc, argv)) > 0)
        join_type = atoi(argv[i + 1]);
    if ((i = ArgPos((char *)"-pos-type", argc, argv)) > 0)
        pos_type = atoi(argv[i + 1]);
    if ((i = ArgPos((char *)"-average-sum", argc, argv)) > 0)
        average_sum = atoi(argv[i + 1]);
